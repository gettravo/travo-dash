...
export default function StackDashboard({ initialApis }: Props) {
    ...
    <div className="space-y-6">
      ...
      >
        ...
        <p className={`text-sm font-medium ${issues.length === 0 ? 'text-green-300' : 'text-yellow-300'}`}>
          {issues.length === 0
            ? 'All your stack services are operational'
            : `${issues.length} service${issues.length !== 1 ? 's' : ''} in your stack experiencing issues`}
        </p>
        {lastUpdated && (
      ...
      <div className="pt-4 border-t border-white/10 flex items-center justify-between">
        <p className="text-sm text-gray-600">
          Monitoring {apis.length} services from your stack
        </p>
        <Link
          href="/stack/edit"
